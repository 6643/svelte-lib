...
- [src/builder/build-internals.ts](../../../src/builder/build-internals.ts) 的 `createBootstrapSource()` 当前通过 `document.getElementById(mountId)` 查找目标, 找不到时抛出异常.
- build、SSE dev 和 native dev 都通过 `createBootstrapSource()` 生成入口, 因此共享该函数即可统一三条应用挂载路径.
- `createRuntimeModuleSource()` 已生成带配置 ID 的运行时目标读取函数, 但目前仍使用“找不到即抛错”的语义.
- `src/builder/svelte-plugin.ts` 是 Bun 编译插件, 负责把 `.svelte` 与 rune module 编译为浏览器 JavaScript; 它不应在 Bun 的 `setup`/`onLoad` 阶段访问页面 DOM.
...
挂载解析逻辑集中在 builder internals 的共享 source generator 中, 由以下两个生成入口复用:
...
DOM 创建和组件挂载属于浏览器运行时入口, 不属于 Bun plugin 的 `setup` 或 `onLoad`. 需要挂载组件的入口仍使用 Svelte 5 的原生形式:

```ts
mount(App, { target });
```

本次只统一 builder 生成入口与生成 runtime module 的目标解析行为, 不增加一层新的运行时组件 API, 也不让编译插件隐式执行浏览器副作用.
...
2. generated runtime behavior: 在 browser/jsdom document 中执行生成的目标解析逻辑, 分别验证已有目标复用和缺失目标创建.
3. integration regression: build、SSE dev、native dev 继续生成原有 HTML mount root, 现有 plugin pipeline、demo check、全量测试和 typecheck 保持通过.
