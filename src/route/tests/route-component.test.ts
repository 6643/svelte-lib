...
import { loadCompiledComponent } from './compile-svelte.ts';
...
describe('Route component', () => {
  test('renders wildcard route when no exact path matches', async () => {
    const Route = await loadCompiledComponent('../Route.svelte', import.meta.url);
    const NotFound = await loadCompiledComponent('./fixtures/NotFound.svelte', import.meta.url);
  ...
  test('renders an exact route match and injects decoded query props', async () => {
    ...
    const Route = await loadCompiledComponent('../Route.svelte', import.meta.url);
    const SyncA = await loadCompiledComponent('./fixtures/SyncA.svelte', import.meta.url);
    const target = document.createElement('div');
    document.body.append(target);
  ...
  test('later duplicate paths override earlier routes', async () => {
    ...
    const Route = await loadCompiledComponent('../Route.svelte', import.meta.url);
    const SyncA = await loadCompiledComponent('./fixtures/SyncA.svelte', import.meta.url);
    const SyncB = await loadCompiledComponent('./fixtures/SyncB.svelte', import.meta.url);
    const target = document.createElement('div');
    document.body.append(target);
  ...
  test('the last wildcard route wins when no exact path matches', async () => {
    cleanupDom();
    cleanupDom = installDom('/missing');
    __resetRouteSystemForTest();
  ...
  test('the active wildcard remains the fallback when exact routes are registered later', async () => {
    cleanupDom();
    cleanupDom = installDom('/missing');
    __resetRouteSystemForTest();
  ...
  test('falls back to the previous wildcard after the active fallback unmounts', async () => {
    cleanupDom();
    cleanupDom = installDom('/missing');
    __resetRouteSystemForTest();
    ...
    const activeWildcard = mount(Route, {
      target,
      props: {
        path: '*',
        component: SyncB
    ...
    flushSync();
    await svelteUnmount(activeWildcard);
    mountedInstances = mountedInstances.filter((instance) => instance !== activeWildcard);
    flushSync();
  ...
  test('rejects non-decoder route config keys such as props', async () => {
    const Route = await loadCompiledComponent('../Route.svelte', import.meta.url);
    const SyncA = await loadCompiledComponent('./fixtures/SyncA.svelte', import.meta.url);
    const target = document.createElement('div');
    ...
    ).toThrow(/Unsupported Route config: props/);
  });

  test('allows $path and $component as decoder keys for child props', async () => {
    cleanupDom();
    cleanupDom = installDom('/user?path=/settings&component=profile');
    __resetRouteSystemForTest();
    ...
    expect(target.querySelector('[data-testid="sync-a"]')?.textContent).toBe('{"path":"/settings","component":"profile"}');
  });
  ...
  test('throws for invalid extra props', async () => {
    ...
    ).toThrow(/Unsupported Route config/);
  });
  ...
  test('throws when component changes after mount', async () => {
    ...
    const MutableRouteHarness = await loadCompiledComponent('./fixtures/MutableRouteHarness.svelte', import.meta.url);
    const target = document.createElement('div');
    document.body.append(target);
  ...
  test('throws when path changes after mount', async () => {
    ...
    const MutableRoutePathHarness = await loadCompiledComponent('./fixtures/MutableRoutePathHarness.svelte', import.meta.url);
    const target = document.createElement('div');
    document.body.append(target);
  ...
  test('throws when decoder changes after mount', async () => {
    ...
    const MutableRouteDecoderHarness = await loadCompiledComponent('./fixtures/MutableRouteDecoderHarness.svelte', import.meta.url);
    const target = document.createElement('div');
    document.body.append(target);
  ...
  test('renders lazy routes without default loading dom', async () => {
    ...
    const Route = await loadCompiledComponent('../Route.svelte', import.meta.url);
    const LazyTarget = await loadCompiledComponent('./fixtures/LazyTarget.svelte', import.meta.url);
    const target = document.createElement('div');
    document.body.append(target);
