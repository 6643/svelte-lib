<script lang="ts">
    ...
    let { label, value, checked = false, changed, disabled = false, name }: Props = $props();
</script>

<label class="btn">
    <input type="radio" {value} {checked} onchange={() => changed?.(value)} {disabled} {name} />
    <span>{label}</span>
</label>
...
<style>
    .btn {
        ...
        user-select: none;
        padding: 6px 12px;
        border: 2px solid var(--sunken-fg);
        border-radius: 3px;
