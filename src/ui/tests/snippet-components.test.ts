...
test("Block renders snippet props into the expected regions", async () => {
    const Harness = await loadCompiledComponent("./fixtures/BlockHarness.svelte", import.meta.url);
...
test("StringInput and RangeInput keep snippet add-ons on the expected sides", async () => {
    const StringHarness = await loadCompiledComponent("./fixtures/StringInputHarness.svelte", import.meta.url);
    const RangeHarness = await loadCompiledComponent("./fixtures/RangeInputHarness.svelte", import.meta.url);
...
test("FilledModal and Swiper render snippet children inside their host containers", async () => {
    const ModalHarness = await loadCompiledComponent("./fixtures/FilledModalHarness.svelte", import.meta.url);
    const SwiperHarness = await loadCompiledComponent("./fixtures/SwiperHarness.svelte", import.meta.url);
